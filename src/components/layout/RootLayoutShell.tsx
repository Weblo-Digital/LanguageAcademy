...
export function RootLayoutShell({ children }: { children: React.ReactNode }) {
  ...
  // Hide header, footer, level test modal, and WhatsApp button on admin, maintenance, and coming-soon pages
  const isAdmin = pathname.startsWith("/admin");
  const isMaintenance = pathname === "/maintenance";
  const isComingSoon = pathname === "/coming-soon";
  const shouldHideLayout = isAdmin || isMaintenance || isComingSoon;
